...
import lcm
from Play.lcm_types.bruce_cmd import bruce_cmd_t
from Play.lcm_types.bruce_data import bruce_data_t
...
def main_loop():
    ...
    # Linear Momentum
    Kp_lm = np.array([100., 50., 150.])
    Kd_lm = np.array([10., 5., 15.])

    #Base Orientation
    Kp_bo = np.array([1000., 1000., 200.])
    Kd_bo = np.array([100., 100., 20.])
    ...
    lc_data = lcm.LCM("udpm://239.255.76.67:7667?ttl=255")
    data_msg = bruce_data_t()
    ...
    while True:
        ...
        data_msg.b_position = Bruce.p_wb
        data_msg.b_velocity = Bruce.v_wb

        lc_data.publish("BRUCE_DATA", data_msg.encode())
