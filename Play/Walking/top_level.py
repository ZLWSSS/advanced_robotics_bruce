...
class UserCommand(object):
    def __init__(self, controller='keyboard', command_frequency=20, display_frequency=10):
        ...
        self.screen_flag = True if 'bruce' in subprocess.check_output(["screen -ls; true"], shell=True).decode(
            "utf-8") else False
        ...
        self.lcm_cmd = bruce_cmd_lidar_t()
            ...
            if PARAMETER_RECOVER[idx] == 'y':
                if np.abs(PARAMETER_DEFAULT[idx] - self.parameter[idx]['value_raw']) > self.tol:
                    ...
                    if self.loop_count % int(self.loop_freq / 20) == 0:  # 20 Hz max
                        self.parameter[idx]['value_raw'] = PARAMETER_DEFAULT[idx] if np.abs(
                            PARAMETER_DEFAULT[idx] - self.parameter[idx]['value_raw']) < PARAMETER_INCREMENT[idx] else \
                        self.parameter[idx]['value_raw'] + np.sign(
                            PARAMETER_DEFAULT[idx] - self.parameter[idx]['value_raw']) * PARAMETER_INCREMENT[idx]
    ...
    def operate(self):
        if self.controller == 'keyboard':
            ...
            if self.target_mode == WALK and self.in_lidar_cmd == True:
                ...
                for idx in PARAMETER_ID_LIST:
                    if self.mode in PARAMETER_MODE_LIST[idx]:
                        if cmd == PARAMETER_BUTTON_PLUS[idx] and PARAMETER_MAX[idx] - self.parameter[idx][
                            'value_raw'] > self.tol:
            ...
            if Bruce.gamepad['ST']:
                ...
                if Bruce.gamepad['FN'] and PARAMETER_MAX[COOLING_SPEED] - self.parameter[COOLING_SPEED][
                    'value_raw'] > self.tol:
                    self.parameter[COOLING_SPEED]['value_raw'] += PARAMETER_INCREMENT[COOLING_SPEED]

                if self.mode == WALK:
                    if Bruce.gamepad['LSP'] and PARAMETER_MAX[FOOT_YAW_LEFT] - self.parameter[FOOT_YAW_LEFT][
                        'value_raw'] > self.tol:
                        self.parameter[FOOT_YAW_LEFT]['value_raw'] += PARAMETER_INCREMENT[FOOT_YAW_LEFT]
                    elif Bruce.gamepad['LSM'] and self.parameter[FOOT_YAW_LEFT]['value_raw'] - PARAMETER_MIN[
                        FOOT_YAW_LEFT] > self.tol:
                        self.parameter[FOOT_YAW_LEFT]['value_raw'] -= PARAMETER_INCREMENT[FOOT_YAW_LEFT]

                    if Bruce.gamepad['RSP'] and PARAMETER_MAX[FOOT_YAW_RIGHT] - self.parameter[FOOT_YAW_RIGHT][
                        'value_raw'] > self.tol:
                        self.parameter[FOOT_YAW_RIGHT]['value_raw'] += PARAMETER_INCREMENT[FOOT_YAW_RIGHT]
                    elif Bruce.gamepad['RSM'] and self.parameter[FOOT_YAW_RIGHT]['value_raw'] - PARAMETER_MIN[
                        FOOT_YAW_RIGHT] > self.tol:
                        self.parameter[FOOT_YAW_RIGHT]['value_raw'] -= PARAMETER_INCREMENT[FOOT_YAW_RIGHT]

                    if Bruce.gamepad['R'] and PARAMETER_MAX[FOOT_CLEARANCE] - self.parameter[FOOT_CLEARANCE][
                        'value_raw'] > self.tol:
                        self.parameter[FOOT_CLEARANCE]['value_raw'] += PARAMETER_INCREMENT[FOOT_CLEARANCE]
                    elif Bruce.gamepad['L'] and self.parameter[FOOT_CLEARANCE]['value_raw'] - PARAMETER_MIN[
                        FOOT_CLEARANCE] > self.tol:
                        self.parameter[FOOT_CLEARANCE]['value_raw'] -= PARAMETER_INCREMENT[FOOT_CLEARANCE]
            else:
                if Bruce.gamepad['FN'] and self.parameter[COOLING_SPEED]['value_raw'] - PARAMETER_MIN[
                    COOLING_SPEED] > self.tol:
                    self.parameter[COOLING_SPEED]['value_raw'] -= PARAMETER_INCREMENT[COOLING_SPEED]

                if self.mode == BALANCE:
                    if Bruce.gamepad['U'] and PARAMETER_MAX[COM_POSITION_X] - self.parameter[COM_POSITION_X][
                        'value_raw'] > self.tol:
                        self.parameter[COM_POSITION_X]['value_raw'] += PARAMETER_INCREMENT[COM_POSITION_X]
                    elif Bruce.gamepad['D'] and self.parameter[COM_POSITION_X]['value_raw'] - PARAMETER_MIN[
                        COM_POSITION_X] > self.tol:
                        self.parameter[COM_POSITION_X]['value_raw'] -= PARAMETER_INCREMENT[COM_POSITION_X]
                    self.parameter[COM_POSITION_Y]['value_raw'] = self.axis2value(-Bruce.gamepad['LX'],
                                                                                  PARAMETER_MAX[COM_POSITION_Y],
                                                                                  PARAMETER_MIN[COM_POSITION_Y])
                    self.parameter[COM_POSITION_Z]['value_raw'] = self.axis2value(Bruce.gamepad['LY'],
                                                                                  PARAMETER_MAX[COM_POSITION_Z],
                                                                                  PARAMETER_MIN[COM_POSITION_Z])

                    self.parameter[BODY_ORIENTATION_X]['value_raw'] = self.axis2value(Bruce.gamepad['RX'],
                                                                                      PARAMETER_MAX[BODY_ORIENTATION_X],
                                                                                      PARAMETER_MIN[BODY_ORIENTATION_X])
                    self.parameter[BODY_ORIENTATION_Y]['value_raw'] = self.axis2value(Bruce.gamepad['RY'],
                                                                                      PARAMETER_MAX[BODY_ORIENTATION_Y],
                                                                                      PARAMETER_MIN[BODY_ORIENTATION_Y])
                    if Bruce.gamepad['RSP'] and PARAMETER_MAX[BODY_ORIENTATION_Z] - self.parameter[BODY_ORIENTATION_Z][
                        'value_raw'] > self.tol:
                        self.parameter[BODY_ORIENTATION_Z]['value_raw'] += PARAMETER_INCREMENT[BODY_ORIENTATION_Z]
                    elif Bruce.gamepad['RSM'] and self.parameter[BODY_ORIENTATION_Z]['value_raw'] - PARAMETER_MIN[
                        BODY_ORIENTATION_Z] > self.tol:
                        self.parameter[BODY_ORIENTATION_Z]['value_raw'] -= PARAMETER_INCREMENT[BODY_ORIENTATION_Z]
                elif self.mode == WALK:
                    self.parameter[COM_VELOCITY_X]['value_raw'] = self.axis2value(Bruce.gamepad['LY'],
                                                                                  PARAMETER_MAX[COM_VELOCITY_X],
                                                                                  PARAMETER_MIN[COM_VELOCITY_X])
                    self.parameter[COM_VELOCITY_Y]['value_raw'] = self.axis2value(-Bruce.gamepad['LX'],
                                                                                  PARAMETER_MAX[COM_VELOCITY_Y],
                                                                                  PARAMETER_MIN[COM_VELOCITY_Y])
        ...
        for idx in PARAMETER_ID_LIST:
            self.parameter[idx]['value_filter'] = MF.exp_filter(self.parameter[idx]['value_filter'],
                                                                self.parameter[idx]['value_raw'], 0.8)
        input_data = {'mode': np.array([self.mode]),
                      'com_xy_velocity': np.array([self.parameter[COM_VELOCITY_X]['value_filter'],
                                                   self.parameter[COM_VELOCITY_Y]['value_filter']]),
                      'yaw_rate': np.array([self.parameter[BODY_YAW_RATE]['value_filter']]) * self.deg2rad,
                      'com_position_change_scaled': np.array([self.parameter[COM_POSITION_X]['value_filter'],
                                                              self.parameter[COM_POSITION_Y]['value_filter'],
                                                              self.parameter[COM_POSITION_Z]['value_filter']]),
                      'body_euler_angle_change': np.array([self.parameter[BODY_ORIENTATION_X]['value_filter'],
                                                           self.parameter[BODY_ORIENTATION_Y]['value_filter'],
                                                           self.parameter[BODY_ORIENTATION_Z][
                                                               'value_filter']]) * self.deg2rad,
                      'right_foot_yaw_angle_change': np.array(
                          [self.parameter[FOOT_YAW_RIGHT]['value_filter']]) * self.deg2rad,
                      'left_foot_yaw_angle_change': np.array(
                          [self.parameter[FOOT_YAW_LEFT]['value_filter']]) * self.deg2rad,
    ...
    def display(self):
        ...
        if self.mode == BALANCE:
            ...
            print('-   lateral (H/J):', self.float2str(self.parameter[COM_POSITION_Y]['value_raw'], 'len'), '[%]')
            print('-  vertical (K/L):', self.float2str(self.parameter[COM_POSITION_Z]['value_raw'], 'len'), '[cm]')
            # if self.in_lidar_cmd:
            #     print('lcm_cmd: com_z: ', self.float2str(self.lcm_cmd.com_z))
            #     print('lcm cmd: vel_com_x ', self.float2str(self.lcm_cmd.com_velocity[0]))
            #     print('lcm cmd: vel_com_y', self.float2str(self.lcm_cmd.com_velocity[1]))
            #     print('lcm cmd: vel_yaw_rate', self.float2str(self.lcm_cmd.ang_vel_yaw))
        print('')
...
def bruce_lidar_handler(channel, data):
    msg = bruce_cmd_lidar_t.decode(data)
    # print('lcm_cmd: com_z: ', msg.com_z)
...
if __name__ == '__main__':
    ...
    uc.lcm.subscribe("bruce_cmd_lidar", bruce_lidar_handler)
    try:
        MM.THREAD_STATE.set({'top_level': np.array([1.0])}, opt='only')  # thread is running
        main_loop()
